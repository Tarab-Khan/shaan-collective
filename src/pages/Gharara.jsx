...
function Gharara({ category = "All", title = "GHARARAS" }) {
  const { womenProducts, heroBanners } = useProducts();
  const heroImage = heroBanners?.women || heroBanners?.home;

  const storageKey = category !== "All" ? `gharara_${category.toLowerCase()}` : "gharara_all";

  const ITEMS_PER_PAGE = 49;
  const minPossiblePrice = 10000;
  const maxPossiblePrice = 250000;
      ...
      <section className="relative overflow-hidden bg-[#070605] px-8 pb-24 pt-40 text-center border-b border-[#dfba6a]/20">
        ...
        <div className="relative z-10 mx-auto max-w-7xl">
          ...
          <p className="mx-auto mt-6 max-w-xl font-serif text-sm sm:text-base italic leading-7 text-[#e8dbbf]/90">
            Discover our collection of handcrafted ghararas,
            created for weddings, celebrations and unforgettable moments.
          </p>
        </div>
      </section>
