...
function MenCollection() {
  const { menProducts, accessoryProducts, heroBanners } = useProducts();
  const heroImage = heroBanners?.men || heroBanners?.home;

  const { occasion } = useParams();
  const storageKey = occasion ? `men_col_${occasion.toLowerCase()}` : "men_col_all";
  const ITEMS_PER_PAGE = 49;

  const minPossiblePrice = 3000;
  const maxPossiblePrice = 60000;
  ...
  const collectionName = names[occasion] || "Men's Collection";

  const allMenProducts = [
    ...(menProducts || []).map((p) => ({ ...p, itemType: "jacket", link: `/men/${p.id}`, wishlistKey: `men-${p.id}` })),
    ...(accessoryProducts || []).filter((p) => p.gender === "Men" || !p.gender).map((p) => ({ ...p, itemType: "accessory", link: `/accessories/${p.id}`, wishlistKey: `accessory-${p.id}` })),
  ];
      ...
      <section className="relative overflow-hidden bg-[#070605] px-8 pb-24 pt-40 text-center border-b border-[#dfba6a]/20">
        ...
        <div className="relative z-10 mx-auto max-w-7xl">
          ...
          <p className="mx-auto mt-6 max-w-xl font-serif text-sm sm:text-base italic leading-7 text-[#e8dbbf]/90">
            Refined formal dressing and accessories crafted for every celebration.
          </p>
        </div>
      </section>
