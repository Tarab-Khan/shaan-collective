...
function MenDetails() {
  ...
  const [selectedSize, setSelectedSize] = useState(() => {
    return sessionStorage.getItem(`men_size_${id}`) || "";
  });
  ...
  useEffect(() => {
    if (selectedSize) {
      sessionStorage.setItem(`men_size_${id}`, selectedSize);
    }
  }, [selectedSize, id]);
  ...
  const wishlistId = `men-${product.id}`;
        ...
        <div className="flex flex-col justify-center">
          ...
          <div>
            ...
            <div className="mt-5 flex flex-wrap gap-3">
              {(product.sizes
                ? product.sizes.map((s) => (s === "CUSTOM" ? "XXL" : s))
                : ["XS", "S", "M", "L", "XL", "XXL"]
              ).map((size) => (
                <button
                  type="button"
                  ...
                  className={`h-12 min-w-14 px-3 border text-xs font-semibold transition ${
                    selectedSize === size
          ...
          <div className="mt-12 space-y-8 border-t border-[#dfba6a]/20 pt-8">
            ...
            <div>
              ...
              <p className="mt-4 text-sm leading-7 text-[#c4b28f]">
                {product.description || "A refined tailored jacket designed for celebrations, weddings and distinguished occasions."}
              </p>
